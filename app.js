...
/* app.js - add 'pabellon' support, remove global clearAll button, add delete directos button */
document.addEventListener('DOMContentLoaded', function() {
  const ADMIN_PASSWORD = window.APP_ADMIN_PASSWORD || 'Pl4s3nc1a!Adm1n#2025';

  // NAV and views (same as before)
  ...
  if(teamPhoto){ teamPhoto.addEventListener('change', async ()=>{ const f = teamPhoto.files[0]; if(!f) return; if(f.size > 6*1024*1024){ alert('Imagen mayor a 6MB'); teamPhoto.value=''; return; } const dataUrl = await fileToDataURL(f); teamPhoto.dataset.dataurl = dataUrl; if(photoPreview) photoPreview.src = dataUrl; if(photoPreviewContainer) photoPreviewContainer.classList.remove('hidden'); }); }

  if(formAddTeam){ formAddTeam.addEventListener('submit', (e)=>{ e.preventDefault(); const idEdit = editingTeamId && editingTeamId.value ? editingTeamId.value : null; const t = { id: idEdit ? Number(idEdit) : Date.now(), name: teamName.value.trim(), club: teamClub.value.trim(), categories: teamCategories.value.trim(), photo: (teamPhoto && teamPhoto.dataset && teamPhoto.dataset.dataurl) ? teamPhoto.dataset.dataurl : null }; if(!t.name){ alert('El nombre del equipo es obligatorio'); return; } let teams = JSON.parse(localStorage.getItem('teams') || '[]'); if(idEdit){ teams = teams.map(tt => tt.id === t.id ? t : tt); if(editingTeamId) editingTeamId.value = ''; if(cancelEditTeam) cancelEditTeam.classList.add('hidden'); if(saveTeamBtn) saveTeamBtn.textContent = 'Guardar equipo'; } else { teams.push(t); } localStorage.setItem('teams', JSON.stringify(teams)); if(formAddTeam) formAddTeam.reset(); if(teamPhoto) teamPhoto.dataset.dataurl = ''; if(photoPreviewContainer) photoPreviewContainer.classList.add('hidden'); renderTeams(); populateMatchTeamsSelects(); renderAdminLists(); }); }
  ...
  function renderTeams(){ const teams = JSON.parse(localStorage.getItem('teams') || '[]'); if(!teamsContainer) return; if(teams.length === 0){ teamsContainer.innerHTML = '<p class="summary">No hay equipos aún.</p>'; return; } const isAdmin = localStorage.getItem('isAdmin') === '1'; teamsContainer.innerHTML = teams.map(t => `
  ...
  if(formAddMatch){ formAddMatch.addEventListener('submit', function(e){ e.preventDefault(); if(matchDateEl && matchDateEl.value){ const picked = new Date(matchDateEl.value); if(picked < new Date()){ alert('La fecha no puede ser en el pasado'); return; } } if(!matchHome.value || !matchAway.value){ alert('Selecciona ambos equipos'); return; } if(matchHome.value === matchAway.value){ alert('Los equipos deben ser distintos'); return; } const idEdit = editingMatchId && editingMatchId.value ? editingMatchId.value : null; const m = { id: idEdit ? Number(idEdit) : Date.now(), homeId: matchHome.value, awayId: matchAway.value, datetime: matchDateEl.value || null, venue: matchVenueEl && matchVenueEl.value ? matchVenueEl.value.trim() : '', result: matchResultEl && matchResultEl.value ? matchResultEl.value.trim() : null }; let matches = JSON.parse(localStorage.getItem('matches') || '[]'); if(idEdit){ matches = matches.map(mm => mm.id === m.id ? m : mm); if(editingMatchId) editingMatchId.value = ''; if(cancelEditMatchEl) cancelEditMatchEl.classList.add('hidden'); if(saveMatchBtnEl) saveMatchBtnEl.textContent = 'Guardar partido'; } else { matches.push(m); } localStorage.setItem('matches', JSON.stringify(matches)); if(formAddMatch) formAddMatch.reset(); renderMatches(); renderAdminLists(); }); }

  if(cancelEditMatchEl) cancelEditMatchEl.addEventListener('click', ()=>{ if(editingMatchId) editingMatchId.value = ''; if(formAddMatch) formAddMatch.reset(); if(cancelEditMatchEl) cancelEditMatchEl.classList.add('hidden'); if(saveMatchBtnEl) saveMatchBtnEl.textContent = 'Guardar partido'; });

  function renderMatches(){ const matches = JSON.parse(localStorage.getItem('matches') || '[]'); const teams = JSON.parse(localStorage.getItem('teams') || '[]'); if(!matchList) return; if(matches.length === 0){ matchList.innerHTML = '<p class="summary">No hay partidos programados.</p>'; return; } const isAdmin = localStorage.getItem('isAdmin') === '1'; matchList.innerHTML = matches.map(m => { const home = teams.find(t => String(t.id) === String(m.homeId)); const away = teams.find(t => String(t.id) === String(m.awayId)); const dateStr = m.datetime ? new Date(m.datetime).toLocaleString() : 'Fecha no asignada'; const venue = m.venue ? '<div style="font-size:13px;color:var(--muted)">Pabellón: ' + escapeHtml(m.venue) + '</div>' : ''; const res = m.result ? '<strong>' + escapeHtml(m.result) + '</strong>' : '<em>Por jugar</em>'; return '<div class="match-item" data-id="' + m.id + '"><div class="match-left"><div><strong>' + escapeHtml(home?home.name:'--') + '</strong> vs <strong>' + escapeHtml(away?away.name:'--') + '</strong></div><div style="font-size:13px;color:var(--muted)">' + dateStr + '</div>' + venue + '</div><div style="display:flex;gap:8px;align-items:center"><div>' + res + '</div>' + ( isAdmin ? '<button class="small-btn edit-match" data-id="' + m.id + '">Editar</button> <button class="small-btn delete-match delete-btn" data-id="' + m.id + '">Eliminar</button>' : '') + '</div></div>'; }).join(''); }

  // DIRECTOS (add delete button functionality)
