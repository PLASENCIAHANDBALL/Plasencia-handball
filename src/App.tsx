...
export default function TournamentApp(){
  const [teams,setTeams]=useState<any[]>([]); const [matches,setMatches]=useState<any[]>([]); const [phases,setPhases]=useState<string[]>(DEFAULT_PHASES); const [events,setEvents]=useState<string[]>(["Torneo 1"]);
  const [filterEvent,setFilterEvent]=useState<string>(ALL); const [filterCategory,setFilterCategory]=useState<string>(ALL); const [filterGroup,setFilterGroup]=useState<string>(ALL); const [search,setSearch]=useState<string>("");
  const [isAdmin,setIsAdmin]=useState(false); const [adminDialogOpen,setAdminDialogOpen]=useState(false); const [pass,setPass]=useState(""); const [adminHash,setAdminHash]=useState("");
  useEffect(()=>{ try{ const raw=localStorage.getItem(LS_KEY); if(raw){ const parsed=JSON.parse(raw); if(parsed?.teams) setTeams(parsed.teams); if(parsed?.matches) setMatches(parsed.matches);} else { const ev="Torneo 1"; const t1={ id:uid(), name:"Plasencia HB", logo:"", categories:["Juvenil Femenino"], club:"Plasencia", notes:""}; const t2={ id:uid(), name:"Coria A", logo:"", categories:["Juvenil Femenino"], club:"Coria", notes:""}; const m1={ id:uid(), event:ev, category:"Juvenil Femenino", group:"Grupo A", phase:"Fase de grupos A", a:t1.id, b:t2.id, datetime:new Date().toISOString(), court:"Pabellón A", scoreA:null, scoreB:null, streamUrl:""}; setTeams([t1,t2]); setMatches([m1]); } const ph=localStorage.getItem(LS_PHASES); if(ph) setPhases(JSON.parse(ph)); const evs=localStorage.getItem(LS_EVENTS); if(evs) setEvents(JSON.parse(evs)); const ah=localStorage.getItem(LS_ADMIN_HASH); if(ah) setAdminHash(ah); setIsAdmin(localStorage.getItem(LS_ADMIN)==="1"); }catch{} },[]);
  useEffect(()=>localStorage.setItem(LS_KEY,JSON.stringify({teams,matches})),[teams,matches]); useEffect(()=>localStorage.setItem(LS_PHASES,JSON.stringify(phases)),[phases]); useEffect(()=>localStorage.setItem(LS_EVENTS,JSON.stringify(events)),[events]); useEffect(()=>localStorage.setItem(LS_ADMIN,isAdmin?"1":"0"),[isAdmin]);
  ...
  const teamsById=useMemo(()=>Object.fromEntries(teams.map((t:any)=>[t.id,t])),[teams]); const allGroups=useMemo(()=>[ALL,...Array.from(new Set(matches.map(m=>m.group).filter(Boolean)))],[matches]);
  ...
  const filteredMatches=useMemo(()=>matches.filter(baseFilter).filter(m=>{ if(!search) return true; const a=teamsById[m.a]?.name||""; const b=teamsById[m.b]?.name||""; return `${a} ${b} ${m.event} ${m.category} ${m.group||""} ${m.phase} ${m.court}`.toLowerCase().includes(search.toLowerCase()); }).sort((x:any,y:any)=>new Date(x.datetime).getTime()-new Date(y.datetime).getTime()),[matches,filterEvent,filterCategory,filterGroup,search,teamsById]);
  const upcomingMatches=useMemo(()=>filteredMatches.filter(m=>new Date(m.datetime)>=new Date()),[filteredMatches]);
  const addTeam=(team:any)=>setTeams(t=>[...t,{id:uid(),...team}]); const removeTeam=(id:string)=>setTeams(t=>t.filter(x=>x.id!==id));
  const addMatch=(nm:any)=>setMatches(p=>[...p,{id:uid(),scoreA:null,scoreB:null,streamUrl:"",notes:"",group:"",...nm}]); const updateMatch=(id:string,patch:any)=>setMatches(p=>p.map(m=>m.id===id?{...m,...patch}:m)); const removeMatch=(id:string)=>setMatches(p=>p.filter(m=>m.id!==id));
  ...
  return (<div className="p-4"><h1 className="text-2xl font-bold">Plasencia Handball</h1><p className="text-sm">App incluida: equipos, partidos, admin y directos. Sube esta carpeta al repo y ejecuta el build.</p></div>);
}
